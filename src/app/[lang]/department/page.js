...
const Designation = () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/department`,
        {
          ...
          body: JSON.stringify({ orgid, departmentid: deleteEmpType }),
        }
      );
      const result = await response.json();
      if (response.ok) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/alldepartment`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        setDesignationData(result?.department);
      }
    } catch (error) {
      console.error(error);
    }
    ...
    >
        ...
        <div className="table-header">
          ...
          <div className="right-column">
            ...
            <a
              ...
              onClick={mountForm}
            >
              <i className="bi bi-plus-circle mr-10"></i>Add Department
            </a>
            <button
      ...
      {openForm && (
        <DepartmentForm
          ...
          formTitle={"Department Name"}
          heading={defaultVal && Object.keys(defaultVal).length > 0 ? "Edit Department" : "Add Department"}
        />
      )}
